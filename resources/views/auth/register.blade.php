<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-6">
        ...
        <!-- NIM -->
        <div>
            <x-input-label for="nim" :value="__('NIM')" class="text-gray-700 dark:text-gray-300 mb-1" />
            <x-text-input id="nim"
                class="block w-full border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-gray-100"
        ...
        <!-- Nama -->
        <div>
            <x-input-label for="name" :value="__('Nama Lengkap')" class="text-gray-700 dark:text-gray-300 mb-1" />
        ...
        <!-- Nomor Telepon -->
        <div>
            <x-input-label for="phone" :value="__('Nomor Telepon')" class="text-gray-700 dark:text-gray-300 mb-1" />
        ...
        <!-- Program Studi -->
        <div>
            <x-input-label for="study_program" :value="__('Program Studi')" class="text-gray-700 dark:text-gray-300 mb-1" />
        ...
        <div class="flex items-center justify-between mt-6">
            <p class="text-sm text-gray-600 dark:text-gray-400">Sudah Punya Akun?
                <a href="{{ route('login') }}"
                    class="font-medium text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 underline">Klik
                    Disini</a>
            </p>
            <x-primary-button
                class="px-6 py-2 bg-blue-600 hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 text-white rounded-md transition-all">{{ __('Buat Akun') }}</x-primary-button>
        </div>
    </form>
</x-guest-layout>
